...
def create_deepchess():
    ...
    chess_network = tf.keras.layers.Dense(2, activation='sigmoid')(chess_network)
...
def train_deepchess(deepchess_model, training_set):
    ...
    deepchess_model.fit({'left_encoder_layer_0': np.array(training_set[0]), 'right_encoder_layer_0': np.array(training_set[1])},{'dense_3': np.array(training_set[2])}, epochs=100, shuffle=True)
