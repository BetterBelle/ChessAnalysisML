...
\begin{document}
    ...
    \begin{abstract}
        We intend to present an automatic learning method for chess, utilizing deep neural networks. We train our deep neural network using a combination of both methods of neural network training, unsupervised and supervised. Using unsupervised training we allow our engine to pretrain which identify and extract high level features that exist in a high level chess match. The supervised training compares two chess position and evaluates the favorable location between both options. Therefore, we create a deep neural network that requires no prior knowledge regardin the rules of chess and strictly learns from thousands of chess datasets. 
        The resulting deep nueral network allows for simulations that extend the possibilies of chess stategies, highly valuable to chess grandmasters and strategist.
    \end{abstract}
